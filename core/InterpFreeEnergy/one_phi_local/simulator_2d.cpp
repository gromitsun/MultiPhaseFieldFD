...

{
    ...
    CHECK_ERROR(clSetKernelArg(_kernel_step_phi_2d, 10, sizeof(Type), &_vars.compa_eq))
    CHECK_ERROR(clSetKernelArg(_kernel_step_phi_2d, 11, sizeof(Type), &_vars.compb_eq))
    CHECK_ERROR(clSetKernelArg(_kernel_step_phi_2d, 12, sizeof(Type), &_vars.L))
    CHECK_ERROR(clSetKernelArg(_kernel_step_phi_2d, 13, sizeof(Type), &_paras.mk))
    CHECK_ERROR(clSetKernelArg(_kernel_step_phi_2d, 14, sizeof(Type), &_paras.kappa))
    CHECK_ERROR(clSetKernelArg(_kernel_step_phi_2d, 15, sizeof(Type), &_paras.dx))
    CHECK_ERROR(clSetKernelArg(_kernel_step_phi_2d, 16, sizeof(Type), &_paras.dt))
...
void Simulator_2D<Type>::steps(const Type dt, const unsigned int nsteps, const bool finish, const bool cputime)
{
    ...
    {
        _paras.dt = dt;
        CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_phi_2d, 16, sizeof(Type), &_paras.dt))
...
//    {
    ...
    /* recalculate parabolic coefficients */
    _vars.a2 = h1 * _para_coef[6 * T_idx] + h2 * _para_coef[6 * T_idx + 6];
    _vars.a1 = h1 * _para_coef[6 * T_idx + 1] + h2 * _para_coef[6 * T_idx + 7];
    _vars.a0 = h1 * _para_coef[6 * T_idx + 2] + h2 * _para_coef[6 * T_idx + 8];
    _vars.b2 = h1 * _para_coef[6 * T_idx + 3] + h2 * _para_coef[6 * T_idx + 9];
    _vars.b1 = h1 * _para_coef[6 * T_idx + 4] + h2 * _para_coef[6 * T_idx + 10];
    _vars.b0 = h1 * _para_coef[6 * T_idx + 5] + h2 * _para_coef[6 * T_idx + 11];
#endif
...
//   _vars.compb_eq = cLeq;
    ...
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_phi_2d, 10, sizeof(Type), &_vars.compa_eq));
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_phi_2d, 11, sizeof(Type), &_vars.compb_eq));
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_phi_2d, 12, sizeof(Type), &_vars.L));
