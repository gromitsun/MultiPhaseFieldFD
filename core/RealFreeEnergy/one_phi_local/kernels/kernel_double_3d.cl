...
// Calculate phase compositions from generic energy functions
...
__kernel void step_3d(// input/output arrays
                      ...
                      const double dt)
{
    ...
    {
        // Read in Phi
        double phia = PhiA[pos-get_global_size(0)*get_global_size(1)*ILZ];
        // Read in Comp
        double comp = Comp[pos-get_global_size(0)*get_global_size(1)*ILZ];
        // Calculate h(phia)
        double phia_sq = sq(phia);
        double phib_sq = sq(1-phia);
        double ha = phia_sq/(phia_sq + phib_sq);
        ...
        local_U[local_pos-local_map] = u;
        
    }
    if (get_local_id(2) == get_local_size(2)-1)
    {
        // Read in Phi
        double phia = PhiA[pos+get_global_size(0)*get_global_size(1)*ISZ];
        // Read in Comp
        double comp = Comp[pos+get_global_size(0)*get_global_size(1)*ISZ];
        // Calculate h(phia)
        double phia_sq = sq(phia);
        double phib_sq = sq(1-phia);
        double ha = phia_sq/(phia_sq + phib_sq);
    ...
    double u_zm = local_U[local_pos-local_map];
    double u_zp = local_U[local_pos+local_map];
