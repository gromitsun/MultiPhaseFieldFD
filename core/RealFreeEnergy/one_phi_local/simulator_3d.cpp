...

{
    ...
    CHECK_RETURN(_kernel_step_3d, clCreateKernel(program, "step_3d", &err), err);
    ...
    CHECK_RETURN(_mem_CompNext,clCreateBuffer(Simulator<Type>::context, CL_MEM_READ_WRITE|CL_MEM_COPY_HOST_PTR, _global_size[0]*_global_size[1]*_global_size[2]*sizeof(Type), _Comp, &err),err)
    
    // Set kernel arguments
    unsigned long int local_ngrids = (_local_size[0]+2)*(_local_size[1]+2)*(_local_size[2]+2);
    
    // input/output arrays
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d,  0, sizeof(cl_mem), &_mem_PhiA))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d,  1, sizeof(cl_mem), &_mem_Comp))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d,  2, sizeof(cl_mem), &_mem_PhiANext))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d,  3, sizeof(cl_mem), &_mem_CompNext))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d,  4, sizeof(Type)*local_ngrids, NULL))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d,  5, sizeof(Type)*local_ngrids, NULL))
    // coefficients of the free energy functions
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d,  6, sizeof(Type), &_vars.a[0]))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d,  7, sizeof(Type), &_vars.a[1]))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d,  8, sizeof(Type), &_vars.a[2]))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d,  9, sizeof(Type), &_vars.a[3]))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d, 10, sizeof(Type), &_vars.a[4]))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d, 11, sizeof(Type), &_vars.b[0]))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d, 12, sizeof(Type), &_vars.b[1]))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d, 13, sizeof(Type), &_vars.b[2]))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d, 14, sizeof(Type), &_vars.b[3]))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d, 15, sizeof(Type), &_vars.b[4]))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d, 16, sizeof(Type), &_vars.b[5]))
    // R * T
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d, 17, sizeof(Type), &_vars.RT))
    // equilibrium compositions
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d, 18, sizeof(Type), &_vars.compa_eq))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d, 19, sizeof(Type), &_vars.compb_eq))
    // partition coefficient (f2a/f2b) at equilibrium compositions (c.f. Steinbach 2006 PRE)
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d, 20, sizeof(Type), &_vars.kba))
    // diffusion coefficients
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d, 21, sizeof(Type), &_paras.Da))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d, 22, sizeof(Type), &_paras.Db))
    // phase field parameters
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d, 23, sizeof(Type), &_vars.L))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d, 24, sizeof(Type), &_paras.mk))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d, 25, sizeof(Type), &_paras.kappa))
    // simulation parameters
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d, 26, sizeof(Type), &_paras.dx))
    CHECK_ERROR(clSetKernelArg(_kernel_step_3d, 27, sizeof(Type), &_paras.dt))
...
void Simulator_3D<Type>::steps(const Type dt, const unsigned int nsteps, const bool finish, const bool cputime)
{
    ...
    {
        _paras.dt = dt;
        CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d, 27, sizeof(Type), &_paras.dt))
    }
    ...
    for (unsigned int t = 0; t<nsteps; t++)
    {
        ...
        CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d, 0, sizeof(cl_mem), &_mem_PhiA))
        CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d, 2, sizeof(cl_mem), &_mem_PhiANext))
        CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d, 1, sizeof(cl_mem), &_mem_Comp))
        CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d, 3, sizeof(cl_mem), &_mem_CompNext))
...
void Simulator_3D<Type>::set_temp()
{
    ...
    // coefficients of the free energy functions
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d,  6, sizeof(Type), &_vars.a[0]));
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d,  7, sizeof(Type), &_vars.a[1]));
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d,  8, sizeof(Type), &_vars.a[2]));
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d,  9, sizeof(Type), &_vars.a[3]));
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d, 10, sizeof(Type), &_vars.a[4]));
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d, 11, sizeof(Type), &_vars.b[0]));
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d, 12, sizeof(Type), &_vars.b[1]));
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d, 13, sizeof(Type), &_vars.b[2]));
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d, 14, sizeof(Type), &_vars.b[3]));
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d, 15, sizeof(Type), &_vars.b[4]));
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d, 16, sizeof(Type), &_vars.b[5]));
    // R * T
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d, 17, sizeof(Type), &_vars.RT));
    // equilibrium compositions
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d, 18, sizeof(Type), &_vars.compa_eq));
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d, 19, sizeof(Type), &_vars.compb_eq));
    // partition coefficient (f2a/f2b) at equilibrium compositions (c.f. Steinbach 2006 PRE)
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d, 20, sizeof(Type), &_vars.kba));
    // phase field parameters
    CHECK_ERROR_EXIT(clSetKernelArg(_kernel_step_3d, 23, sizeof(Type), &_vars.L));
